...
Layer overrides are independent per viewport. Fractional layer values are supported and rendered as integer ranks. Groups derive their default layer from their children. Other Canvas settings are `shape`, `shapeStretch`, `fit`, `verticalAlign`, `imagePosition`, `aspectRatio` (resize lock), `group`, `offset`, and `order`.

`shapeStretch: true` stretches a mask to its frame; `false` preserves its preferred proportions. Omit it to use the shape’s default. Preserve explicit false values. It changes neither the saved frame nor the photo crop. An explicit stretch preference also keeps the current frame when selecting another shape.
