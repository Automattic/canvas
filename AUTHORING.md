...
An image directly inside Canvas can use `fillHeight: true` on an authored viewport placement. Moving or resizing its frame to both outer vertical Canvas edges enables this automatically; padding edges alone do not. Full height inherits with the placement on smaller viewports, while an explicit placement without the flag uses ordinary sizing. The image follows the section's final height, including padding and readable-content growth, without contributing a minimum height itself. It overrides `frameRatio` and the vertical part of `free` while preserving horizontal placement, crop, fill, shape, and rotation. Moving or resizing either vertical edge away releases it and saves the new ordinary frame. Horizontal moves and section-height edits retain it. Omit false `fillHeight`; never save the derived full-height frame or add vertical anchors. Existing edge-to-edge images are unchanged until edited. Nested Group images do not support this behavior.

Adding rows leaves blocks in place with unchanged dimensions, including partial padding cells. Shift-resizing adds equal space above and below and translates grouped children once. Group and multiple-selection moves use one snapped destination and a shared translation, preserving internal spacing. Editing an individual child resolves that child to cells. Sibling guides confirm alignment of the snapped destination without changing placement or creating off-grid frames.

Use `canvas.fill` for the shared **Fill area** setting on headings, paragraphs, and images. `true` scales and wraps text within its authored width and height, or crops an image proportionally to cover its frame; `false` uses normal text sizing or shows the whole image. Preserve explicit boolean values, including `false`. With the setting absent, images fill their frames and text uses normal sizing. Newly inserted headings receive `fill: true`; paragraphs do not. Image shapes require fill while active and retain the saved preference for when the shape is removed. Text **Fill area** retains the authored frame and all eight resize handles. **Fit text** uses native `fitText: true`, preserves one line, derives height from the text, and exposes left/right resize handles. The two settings are mutually exclusive: enabling either disables the other. Derived height is never saved automatically. Full-height attachment remains an independent per-viewport placement setting. The former `fitArea` and `fit` fields are unsupported. Omit zero rotation, empty anchor/layer maps, and defaults `shape: "none"` and `verticalAlign: "top"`. Save positive image `frameRatio` values to six significant digits. Round free-frame `x`, `y`, `width`, and `ratio` to six decimal places when saving; preserve tiny positive widths and ratios if rounding would make them zero.

Layer overrides are independent per viewport. Fractional layer values are supported and rendered as integer ranks. Groups derive their default layer from their children. Other Canvas settings are `shape`, `shapeStretch`, `verticalAlign`, `imagePosition`, `aspectRatio` (resize lock), `group`, `offset`, and `order`. Shape proportions and frame resizing are independent: `shapeStretch: false` preserves the silhouette inside the frame without locking resizing. Only the explicit aspect-ratio lock or a temporary resize modifier constrains the frame ratio.

For an image with a shape, the **Fill area** control reads and writes `shapeStretch`: `true` stretches the mask to its frame; `false` preserves its preferred proportions. The photo always fills the shape without distortion. The saved `canvas.fill` preference stays unchanged and takes effect again when the shape is removed. All shapes default to preserving their proportions. Preserve explicit false values. It changes neither the saved frame nor the photo crop position. Selecting a different shape clears the stretch override and preserves the existing cell dimensions.

Empty image placeholders always occupy their frame. Their **Fill area** control is checked and disabled until an image URL is present, including when a shape is selected. This temporary editor state does not change saved `fill` or `shapeStretch` preferences; adding an image restores the applicable saved preference.
