...
The resulting `dist/canvas-preview.zip` contains a Blueprint and the compiled `canvas.zip`. The Blueprint installs Twenty Twenty-Five and Canvas, creates a homepage from the registered **pattern-1** pattern using the theme's **Page No Title** template, disables editor welcome guides, logs the visitor in, and opens that page in the Site Editor with **Show template** enabled. The pattern remains ordinary editable blocks. Change the pattern name in [scripts/setup-preview.php](scripts/setup-preview.php) to choose a different bundled composition.
