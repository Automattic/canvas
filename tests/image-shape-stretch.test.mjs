...
test('stretch fills every shape frame and preserving proportions uses each shape ratio', () => {
  for (const shape of shapes) for (const shapeStretch of [true, false]) {
    ...
    assert.equal(imageResizeRatio(layout, frame), shapeStretch ? undefined : preferredShapeRatio(shape));
...
test('stretch releases automatic shape locking but preserves explicit and temporary resize locks', () => {
  ...
  assert.equal(imageResizeRatio(resolve({ shape: 'ellipse', shapeStretch: false, aspectRatio: 1.7 }), frame), 7 / 6);
  for (const invalid of [null, 'true', 0, 1, {}]) {
    assert.equal(imageShapeStretch({ shape: 'circle', shapeStretch: invalid }), false);
    assert.equal(imageShapeStretch({ shape: 'ellipse', shapeStretch: invalid }), true);
...
test('stretch preferences survive shape changes and keep an explicitly stretched frame in place', () => {
  ...
  assert.equal(proposed.canvas.shapeStretch, true);
  ...
  assert.notDeepEqual(fitted.canvas.desktop, attributes.canvas.desktop);
  assert.equal(fitted.canvas.shapeStretch, false);
});
